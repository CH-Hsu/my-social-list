...
function renderUserData(data) {
  ...
  data.forEach(item => {
    ...
    if (isAddedToLike(Number(item.id))) tempHTML += '<i class="fas fa-heart" style="color:#e64c48;"></i>'
...
function showMoreUserInfo(id) {
  ...
  axios.get(INDEX_URL + id).then(response => {
    ...
    if (isAddedToLike(Number(id))) {
      ...
      modalBtn.children[1].innerHTML = '<i class="fas fa-heart-broken"></i> Delete'
    } else {
...
function deleteFromLike(id) {
  ...
  if (userIndex === -1) return alert('The user doesn\'t exist in your like list!')
